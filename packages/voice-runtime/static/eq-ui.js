...
(function (global) {
  ...
  const GMIN = -12;
  const GMAX = 12;
  const COLORS = ["#8b5cf6", "#a78bfa", "#7c3aed", "#c084fc", "#6366f1", "#a855f7", "#d8b4fe"];
  ...
  class EqVisualizer {
    ...
    _drawGrid() {
      ...
      for (let db = GMIN; db <= GMAX; db += 3) {
        ...
        ctx.strokeStyle = db === 0 ? "rgba(139,92,246,0.22)" : "rgba(139,92,246,0.06)";
    ...
    _drawSpectrum() {
      ...
      ctx.strokeStyle = "rgba(196,181,252,0.5)";
      ctx.lineWidth = 1.3;
      ctx.stroke();
    }
    ...
    draw() {
      ...
      if (!this.enabled || !this.bands.length) {
        ...
        this.ctx.fillText(this.enabled ? "Flat — select a preset" : "EQ bypassed", this.w / 2, this.h / 2);
        return;
      }
      this._drawCurve(this.bands, "rgba(196,181,252,0.92)", null);
